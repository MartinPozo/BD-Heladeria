...
def ConsultarVentas():
    ...
    while VerificarFecha(fecha_inicio) == False:
        fecha_inicio = input('Fecha en formato Erroneo (AAAA-MM-DD) (Presione X para cancelar): ')
        if fecha_inicio.lower() == 'x': return

    while True:
        fecha_fin = input('Fecha de fin (AAAA-MM-DD) (Presione X para cancelar): ')
        if fecha_fin.lower() == 'x': return
        while VerificarFecha(fecha_fin) == False:
                fecha_fin = input('Fecha en formato Erroneo (AAAA-MM-DD) (Presione X para cancelar): ')
                if fecha_fin.lower() == 'x': return
...
def AñadirLoteProductos(Id):
    ...
    while True:
        ...
        while id_maquina.isnumeric() != True:
            id_maquina = input("El ID debe ser numerico Ingresar otra vez (X para cancelar): ")
            if id_maquina.lower() == 'x': return
    ...
    while True:
        ...
        while id_producto.isnumeric() != True:
            id_producto = input("El ID debe ser numerico Ingresar otra vez (X para cancelar): ")
            if id_producto.lower() == 'x': return
...
def ConsultarProductos(Id):
    ...
    print(f"\n--- PRODUCTOS DISPONIBLES EN SUCURSAL (ID: {Id}) ---")
    if not filas:
        print("No se encontraron productos asociados a las máquinas de esta sucursal.")
    else:
        for fila in filas:
            vegano = "Sí" if fila[3] else "No"
            receta_corta = (fila[4][:50] + "...") if fila[4] else "Sin receta"
            
            print(f"ID: {fila[0]} | Producto: {fila[1]} | Precio: ${fila[2]} | Vegano: {vegano} | Receta: {receta_corta}")
...
def ConsultarLotes(Id):
    """
    Consulta y muestra todos los lotes de productos asociados a una sucursal específica.
    """
    sql = """
    SELECT 
    ...
    print(f"\n--- DETALLE DE LOTES (SUCURSAL ID: {Id}) ---")
    if not filas:
        print("No se encontraron lotes registrados para esta sucursal.")
    else:
        for fila in filas:
            f_elab = fila[4] if fila[4] else "N/A"
            f_venc = fila[5] if fila[5] else "N/A"
            
            print(f"Lote N°: {fila[0]} | Producto: {fila[1]} | Stock: {fila[2]} ud. | Costo Lote: ${fila[3]} | F. Elab: {f_elab} | F. Venc: {f_venc} | Máquina ID: {fila[6]}")
...
def AñadirMaquina(Id):
    print('--- CARACTERISTICAS DE LA MAQUINA --- \n')
    sabor = input('Ingrese sabor (X para cancelar): ')
    if sabor.lower() == 'x': return
    
    while True:
        consumo = input('Ingrese Consumo en Watts (X para cancelar): ')
...
def EditarCaja(Id):
    ...
    if opcion == '1':
        ...
        if estado.lower() == 'x': return
        if estado == '1': 
            print('La caja ha sido abierta con exito...')
            estado = True
        else: 
            print('La caja ha sido cerrada con exito...')
            estado = False
        Ejecutar(
...
def validar_y_formatear_hora(hora_str):
    """
    Valida que el string cumpla con los rangos de HH:MM:SS 
    y autocompleta si faltan componentes (ej: '8' -> '08:00:00').
    Devuelve la hora formateada si es válida, o None si no lo es.
    """
    partes = hora_str.split(':')
    
    if len(partes) == 1:
        partes = [partes[0], '00', '00']
    elif len(partes) == 2:
        partes = [partes[0], partes[1], '00']
    elif len(partes) != 3:
        return None
...
def AñadirEmpleado(Id):
    ...
    Ejecutar(
        "UPDATE sucursal SET numero_empleados = numero_empleados + 1 WHERE id_sucursal = %s",
        (Id,)
    )
    sql_con = """
    ...
    while True:
        ...
        while True:
            entrada_raw = input(f'Hora de entrada estimada para el {dia} (HH:MM:SS): ').strip()
            if entrada_raw.lower() == 'x': 
                return
            
            entrada = validar_y_formatear_hora(entrada_raw)
            if entrada:
                break  
            print("Hora inválida. Use el formato HH:MM:SS")

        while True:
            salida_raw = input(f'Hora de salida estimada para el {dia} (HH:MM:SS): ').strip()
            if salida_raw.lower() == 'x': 
                return
                
            salida = validar_y_formatear_hora(salida_raw)
            if salida:
                break  
            print("Hora inválida. Use el formato HH:MM:SS")
...
def Menu():
    ...
    while True:
        ...
        if opcion == '1':
            ...
            Id = input("Introduzca ID (Presione X para cancelar): ")
...
def MenuProductos(Id):
    ...
    while True:
        ...
        print('-----------------------------')
        print('3. Consultar Lote de Productos')
        print('4. Añadir Lote de Productos')
        print('5. Eliminar Lote')
        print('-----------------------------')

        print('6. Añadir Producto')
        print('7. Modificar Producto')
        print('8. Consultar stock')
        print('9. Consultar Productos')
        print('-----------------------------')
        print('X. Salir \n')
